...
do_configure () {
	...
	chmod +x ${S}/configure
	unset CFLAGS
	${S}/configure --host=${TARGET_SYS} --target=${TARGET_SYS} --build=${BUILD_SYS} \
		--without-cvs --disable-sanity-checks --disable-profile --disable-debug --without-gd --without-selinux \
